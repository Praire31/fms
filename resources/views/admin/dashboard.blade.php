...
<div class="dashboard-container">
    ...
    {{-- 👇 keep you on the Users tab after reload --}}
    <input type="hidden" name="tab" value="users">

    <input type="text" name="search" placeholder="Search users..."
           value="{{ request('search') }}" class="input-field" />
    <button type="submit" class="btn search-btn">Search</button>

    {{-- 👇 also keep the Users tab when clearing --}}
    <a href="{{ route('admin.dashboard', ['active_tab' => 'users']) }}" class="btn clear-btn">Clear</a>
</form>

                </div>

                {{-- Add User Form --}}
                <form id="addUserForm" method="POST" action="{{ route('admin.users.store') }}" style="display:none;margin-bottom:20px;">
                    @csrf
                    ...
                    <input type="password" name="password" placeholder="Password" required style="padding:8px;margin-right:5px;">
                ...
                <form id="editUserForm" method="POST" style="display:none;margin-bottom:20px;">
                    ...
                    <input type="text" name="name" id="editUsername" placeholder="Full Name" required style="padding:8px;margin-right:5px;">
                    <input type="email" name="email" id="editEmail" placeholder="Email" required style="padding:8px;margin-right:5px;">
                    ...
                    <tbody>
                        @foreach($users as $index => $user)
                            <tr>
                                ...
                                <td>
                                    <!-- Edit Button -->
                                    <a href="#" class="btn btn-warning btn-sm" onclick="showEditUserForm({{ $user->id }}, '{{ $user->name }}', '{{ $user->email }}', {{ $user->department_id ?? 'null' }})">Edit</a>
        ...
        <div id="departments" class="content-section">
            <div class="card">
                ...
                <div class="department-actions" style="margin-bottom:15px;">
                    ...
                    <form id="addDeptForm" method="POST" action="{{ route('admin.departments.store') }}" style="display:none;">
                        @csrf
                        <input type="text" name="name" placeholder="Department Name" required style="padding:8px;margin-right:5px;">
...
<form id="editDeptForm" method="POST" style="display:none;margin-bottom:20px;">
    ...
    <input type="text" name="name" id="editDeptName" placeholder="Department Name" required style="padding:8px;margin-right:5px;">
                    ...
                    <tbody>
                        @foreach($departments as $index => $dept)
                            <tr>
                                ...
                                <td>{{ $dept->name }}</td>
                                <td>
                                    <button class="btn btn-warning btn-sm" onclick="showEditDeptForm({{ $dept->id }}, '{{ $dept->name }}')">Edit</button>
                    ...
                    <form method="GET" action="{{ route('admin.dashboard') }}" class="flex gap-2 flex-wrap">
                        <select name="filter_user" class="input-field">
                            ...
                            @foreach($usersForFilter as $user)
                                <option value="{{ $user->name }}" {{ request('filter_user') == $user->name ? 'selected' : '' }}>{{ $user->name }}</option>
                            @endforeach
                        </select>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // If no tab in URL, fall back to Laravel-provided value
    if (!activeTab) {
